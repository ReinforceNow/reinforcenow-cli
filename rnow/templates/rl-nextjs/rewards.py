    ...
    try:
        ...
        matches = root.find_all(Config(rule={
            "all": [
                ...
                {"kind": "function_declaration"},
                {"has": {"pattern": "const $VAR = (await searchParams).$VAR", "stopBy": "end"}},
            ]
        }))
        return 1.0 if matches else 0.0
    except Exception:
...
async def metadata_export(args, sample, **kwargs) -> float:
    ...
    try:
        root = SgRoot(tsx_code, "tsx").root()
        matches = root.find_all(Config(rule={
            "pattern": "export const metadata = { $$$BODY }"
        }))
        return 1.0 if matches else 0.0
    except Exception:
        return 0.0
...
async def not_found_boundary(args, sample, **kwargs) -> float:
    ...
    try:
        root = SgRoot(tsx_code, "tsx").root()
        matches = root.find_all(Config(rule={
            "pattern": "export default function NotFound() { $$$BODY }"
        }))
        return 1.0 if matches else 0.0
    except Exception:
        return 0.0
...
async def loading_boundary(args, sample, **kwargs) -> float:
    ...
    try:
        root = SgRoot(tsx_code, "tsx").root()
        matches = root.find_all(Config(rule={
            "pattern": "export default function Loading() { $$$BODY }"
        }))
        return 1.0 if matches else 0.0
    except Exception:
        return 0.0
