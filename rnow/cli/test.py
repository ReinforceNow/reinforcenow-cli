...
Uses the /api/rnow/rollout endpoint which runs the same env.py
rollout logic as production training (single source of truth).
...
class RolloutClient:
    ...
    Uses the same env.py rollout logic as production training,
...
def test(
    ...
    This command runs RL rollouts by calling the /api/rnow/rollout endpoint,
    which uses the same env.py rollout logic as production training.

    Entries with a 'docker' field will automatically use Modal sandboxes
    for @tool(sandbox=True) and @reward(sandbox=True) execution.
...
async def _test_async(
    ...
    env_py_code = env_path.read_text() if with_tools and env_path.exists() else None
