...
Uses tinker-cookbook for tokenization/rendering, sends tokens to Next.js API
which calls Tinker with secure API key, then decodes response.

Flow: rnow test -> tokenize with tinker-cookbook -> POST to Next.js /api/rnow/sample
      -> Next.js calls Tinker API -> returns tokens -> decode with tinker-cookbook
"""
...
from rnow.core.reward import REWARD_REGISTRY, clear_reward_registry
...
class TinkerCompleter:
    ...
    async def __call__(self, messages: list[dict], stop: list[str] | None = None) -> dict:
        ...
        return {"content": parsed_message.get("content", "")}
...
async def _run_single_rollout(
    ...
    while turn_count < max_turns:
        ...
        if verbose:
            click.echo(f"  [Turn {turn_count}] {_format_message({'role': 'assistant', 'content': response_content}, max_len=500)}")
        ...
        for raw_call in tool_matches:
            ...
            try:
                ...
                if verbose:
                    click.echo(f"    Tool {click.style(tool_name, fg='cyan')}: {str(tool_result)[:200]}")
        ...
        if termination_policy == "last_tool":
    ...
    rewards = {}
    for fn, name in zip(reward_fns, reward_names):
        value = await fn(reward_args, conversation)
        rewards[name] = value

    total_reward = sum(rewards.values()) / len(rewards) if rewards else 0.0
...
def test(ctx, project_dir, num_rollouts, multi_turn, with_tools, model, api_url, verbose, truncate):
    ...
    asyncio.run(
        _test_async(
            ...
            api_url=api_url or ctx.obj.get('api_url', '').replace('/api', '') or DEFAULT_API_URL,
            verbose=verbose,
            truncate=truncate,
        )
    )
...
async def _test_async(
    ...
    spinner = Spinner(f"Loading tokenizer for {model_name}")
    spinner.start()
    ...
    try:
        completer = TinkerCompleter(api_base=api_url, model=model_name, max_tokens=max_tokens)
        spinner.stop()

        for i in range(num_rollouts):
            sample = random.choice(samples)

            # Start spinner for this rollout
            spinner = Spinner(f"Rollout {i+1}/{num_rollouts}")
            spinner.start()
            ...
            try:
                ...
                click.echo(f"Rollout {i+1}/{num_rollouts}")
    ...
    if rewards:
        ...
        click.echo()
        click.echo(click.style(f"Mean reward over {len(rewards)} rollouts: {mean_reward:.3f}", fg="cyan", bold=True))
    else:
